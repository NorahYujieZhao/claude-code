...
function IDEScreen(t0) {
  ...
  if ($[17] !== availableIDEs.length) {
    t5 = availableIDEs.length === 0 && <Text dimColor={true}>{isSupportedJetBrainsTerminal() ? "No available IDEs detected. Please install the plugin and restart your IDE:\nhttps://docs.claude.com/s/claude-code-jetbrains" : "No available IDEs detected. Make sure your IDE has the yyyjie-compatible extension or plugin installed and is running."}</Text>;
  ...
  let t7;
  if ($[24] !== availableIDEs) {
    t7 = availableIDEs.length !== 0 && availableIDEs.some(_temp2) && <Box marginTop={1}><Text color="warning">Note: Only one yyyjie instance can be connected to VS Code at a time.</Text></Box>;
    $[24] = availableIDEs;
    $[25] = t7;
  } else {
    t7 = $[25];
...
export async function call(onDone: (result?: string, options?: {
  ...
  if (args?.trim() === 'open') {
    ...
    if (availableIDEs.length === 0) {
      onDone('No IDEs with a yyyjie-compatible extension detected.');
      return null;
    }
