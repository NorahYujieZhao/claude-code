...
import { CLI_NAME, PRODUCT_NAME } from '../../constants/brand.js'
import {
...
export function registerMcpAddCommand(mcp: Command): void {
	mcp
		...
		.addOption(
			new Option(
				'--xaa',
				`Enable XAA (SEP-990) for this server. Requires '${CLI_NAME} mcp xaa setup' first. Also requires --client-id and --client-secret (for the MCP server's AS).`,
			...
			if (!name) {
				cliError(
					'Error: Server name is required.\n' +
						`Usage: ${CLI_NAME} mcp add <name> <command> [args...]`,
				)
			...
			try {
				...
				if (xaa) {
					...
					if (!getXaaIdpSettings()) {
						missing.push(`'${CLI_NAME} mcp xaa setup' (settings.xaaIdp not configured)`)
					}
					if (missing.length) {
				...
				if (transport === 'sse') {
					...
					if (!transportExplicit && looksLikeUrl) {
						...
						process.stderr.write(
							`If this is an HTTP server, use: ${CLI_NAME} mcp add --transport http ${name} ${actualCommand}\n`,
						)
						process.stderr.write(
							`If this is an SSE server, use: ${CLI_NAME} mcp add --transport sse ${name} ${actualCommand}\n`,
						)
					}
